...
(s/defn ^:private maybe-infer-special-type :- ResultColumnMetadata
  "If we already have a special type from the stored metadata, use that, otherwise compute the special type similar to
  how our sync process would compute it for a new field."
  [result-metadata col]
  (if (contains? result-metadata :special_type)
    result-metadata
    (assoc result-metadata :special_type (classify-name/infer-special-type col))))
